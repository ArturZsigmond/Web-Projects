...
<body>
  <div class="top-bar">
    ...
    &nbsp;&nbsp;
    <!-- Shuffle button always visible when not solved -->
  ...
  <!-- Hidden form for swapping or click-move -->
  ...
  <script>
    // ──────────────────────────────────────────────────────────────────────────────
    // 0) Inject “isSolved” from JSP so JS sees a true/false literal.
    //    At runtime this becomes either:
    //        const isSolved = true;
    //    or  const isSolved = false;
    <% if (Boolean.TRUE.equals(solved)) { %>
      const isSolved = true;
    <% } else { %>
    ...
    // 1) Attach dragstart/dragend to every <img> inside .grid-item
    ...
    // 2) Prevent default on dragover for each .grid-item (so we can drop)
    document.querySelectorAll(".grid-item").forEach(div => {
      div.addEventListener("dragover", function (evt) {
        evt.preventDefault();
      });
      // 3) On drop: get targetIndex, send to servlet as “swap”
    ...
    // 4) “Click‐to‐move‐into‐blank” fallback: if user clicks a tile, we send action="clickMove"
    document.querySelectorAll(".grid-item img").forEach(img => {
      img.addEventListener("click", function () {
        ...
        // Send only fromIndex; server will locate blankIndex and swap if adjacent.
